...
class TestCaseCommon(object):
    ...
    # 根据路径更新yaml文件
    yaml_data = read_testcase_yaml("/yaml_data/organization_yaml/saveOrganization.yaml")
    ...
    def test_method(self, kwargs):
        ...
        for item in execute_sql.get("setup", "") :
            if item.get("type") == "api" :
                self.http_client.request(**item.get("data"))
            elif item.get("type") == "db" :
                self.mysql_client.execute(item.get("sqls"))
